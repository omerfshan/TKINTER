...
def selectItem(e):
    selected_item=tvw.focus()
    people=tvw.item(selected_item)["values"]
    
    txt_firstName.delete(0, END)
    txt_lastName.delete(0, END)
    txt_email.delete(0, END) 

    txt_firstName.insert(0, people[1])
    txt_lastName.insert(0, people[2])
    txt_email.insert(0, people[3]) 
...
def updatePeople():
    ...
    connection=sqlite3.connect("example.db")
    cursor=connection.cursor()
    cursor.execute("UPDATE people SET first_name=:ad, last_name=:soyad, email=:email WHERE id=:id",{
        "id":id,
        "ad":ad,
        "soyad":soyad,
        "email":email
...
btn_update = tb.Button(add_update_frame, text="Güncelle", bootstyle="secondary outline", command=updatePeople)
